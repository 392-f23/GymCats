export const getNaiveMatches = (allUserData, personalUserId) => {
  ...
  for (var user in allUserData) {
    if (
      isInvalidMatch(
        user,
        personalUserId,
        personalUserData.Friends,
        personalUserData.NotInterested
      )
    )
      continue;
    uids.push(user);
  ...
  for (var user in userPersonalData) {
    ...
    if (numMatchingFields >= numMatchingFieldsRequired) {
      matches.push(Object.assign(allUserData[user], { uid: user }));
    }
  }
...
const isInvalidMatch = (
  ...
  personalUserId,
  personalUserFriends,
  personalUserNotInterested
) => {
  return (
    otherUser == personalUserId ||
    personalUserFriends.includes(otherUser) ||
    personalUserNotInterested.includes(otherUser)
  );
};
